...
class DataEnlarger():
    def __init__(self, llm, embd, data_path):
        self.llm = llm
        self.embd = embd
        self.leaf_chunks = create_chunks(data_path)
        self.chunks = self.enlarge_chunks()

    def enlarge_chunks(self, n_levels = 2, level = 1):
        #n_levels - сколько уровней мы делаем в дереве, если идти снизу вверх
        #level с какого уровня мы начинаем (1 - с листьев)
        results = self.recursive_embed_cluster_summarize(self.leaf_chunks, level=level, n_levels=n_levels)

        all_chunks = self.leaf_chunks.copy()
    ...
    def embed_cluster_summarize_texts(self,
        ...
        df_clusters = embed_cluster_texts(texts, self.embd)
